...
export function InventoryPanel() {
  ...
  const selectedItem = selectedInventoryItemId ? items.find((item) => item.id === selectedInventoryItemId) ?? null : null;
  ...
  return (
    <section className="panel hud-panel rounded-[28px] p-4">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.28em] text-amber-200/60">Inventory</p>
          <h3 className="mt-1 text-lg font-semibold text-amber-50">인벤토리</h3>
        </div>
        <span className="hud-chip px-3 py-1 text-[11px] font-semibold text-amber-100/75">28칸</span>
      </div>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1 truncate text-amber-50">{equipped?.name ?? "비어 있음"}</div>
                </div>
              </div>
            </button>
          );
      ...
      <div className="grid grid-cols-4 gap-2">
        {Array.from({ length: 28 }).map((_, index) => {
          ...
          const canUse =
            itemData?.type === "consumable" || itemData?.id === "return_scroll" || itemData?.id === "teleport_scroll";
          ...
          return (
            <button
              ...
              className={`aspect-square rounded-2xl border bg-black/25 p-2 text-left text-xs transition ${
                active ? "border-amber-400 shadow-[0_0_24px_rgba(251,191,36,0.18)]" : "border-amber-200/10 hover:border-amber-200/25"
              }`}
            >
              {item && itemData ? (
                <div className="flex h-full flex-col justify-between">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className={`line-clamp-2 text-right font-medium leading-4 ${rarityClass[item.rarity]}`}>{item.name}</div>
                  </div>
        ...
        <div className="mt-4 rounded-[24px] border border-amber-200/10 bg-black/25 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-100/50">{selectedItemData.icon}</p>
            </div>
          </div>
          <p className="mt-2 text-xs leading-5 text-amber-100/70">{selectedItemData.description}</p>
